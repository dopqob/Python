#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/12/17 18:33
# @Author  : Bilon
# @File    : email_manager.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/12/17 17:51
# @Author  : Bilon
# @File    : email.py
import mimetypes
import os
...
class EmailManager(object):
    ...
    def get_main_msg(self):
        ...
        msg['Cc'] = self._format_addr('To <%s>' % self.msg_cc)    # 抄送人
    ...
    def get_attach_file(self):
        ...
        if self.attach_file is not None and self.attach_file != '':
            try:
                if getsize(self.attach_file) > self.MAX_FILE_SIZE:  # 附件大于10M抛出异常
...
if __name__ == "__main__":
    mail_cfgs = {'msg_from': '30884413@qq.com',     # 发送邮箱
                'password': 'latimgqpunpgcbeb',     # 邮箱密码（授权码）
                'msg_to': ['1335150450@qq.com', '1551658080@qq.com'],    # 接收邮箱
                'msg_cc': [],    # 抄送邮箱
                'msg_subject': 'Python Auto Send Email Test',   # 邮件主题
                'msg_content': 'Hi, boy! Just do it, python!',  # 邮件内容
                'attach_file': r'.\README.md',  # 附件
